...
link:getting-started-client-tools.html[OpenShift client tools (rhc)].
...
* Install Git and the link:getting-started-client-tools.html[OpenShift Client Tools (RHC)] if you
...
* Type *yes* when the wizard asks if you want to upload your public key. Your public key will be
uploaded and stored on the server.

IMPORTANT: For Windows users, `rhc setup` will only upload your public key if you've already insatlled link:http://msysgit.github.io/[Git for Windows]. Alternatively, you can use link:#download-the-putty-installer[PuTTY] to connect to your application with SSH.
...
== Installing an SSH Client
Most UNIX-like platforms like Mac OS X and Linux have SSH installed by default. However, Windows does not. Many third-party SSH clients for windows are available, however we prefer PuTTY, which is free to use.

link:#using-ssh-on-windows[-> Install and Configure PuTTY for SSH on Windows]
...
The link:#using-ssh-on-windows[Windows
...
link:https://openshift.redhat.com/app/console/applications[applications list
...
|This section describes a list of common commands that can help you to manage and debug your applications once you have SSH'ed into your application.
...
* How to resolve
https://help.openshift.com/hc/en-us/articles/202398530[permission
