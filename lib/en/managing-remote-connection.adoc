...
[[setting-up-ssh-keys]]
== Setting up SSH Keys
When the link:managing-client-tools.html[client tools are installed] and the `rhc setup` command is initially run to configure the client tools, the setup wizard generates a new pair of SSH keys in the default `.ssh` folder of your home directory.
